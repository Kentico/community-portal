...
public class BlogSearchIndexingStrategy : DefaultLuceneIndexingStrategy
{
    public override async Task<LuceneSearchModel> OnIndexingNode(IndexedItemModel lucenePageItem, LuceneSearchModel model)
    {
        ...
        if (page is not null)
        {
            ...
            var blogImg = await assetService.RetrieveMediaFileImage(page.BlogPostPageTeaserMediaFileImage.FirstOrDefault());
            ...
            var date = page.BlogPostPageDate != default
                ? page.BlogPostPageDate
                : DateTime.MinValue;
